...
#include<cmath>
using namespace std;
class event                                                                         //created base class of event
{
    public:
    string enme,Fname,Lname;
    int min , guestno;
    int userinfo()                                                                 //getting user requriment using userinfo()
    {
...
class estimate : public event                                       //created derived class of esitimate inherited from event class
{
    public:
    int x;
    float numofserver;
    ...
    int showdata()
    {
         numofserver = guestno / 20;                              //1 server = 20 person

         cost1 = (min /60) * CostPerHour;
         cost2 = (min % 60) * CostPerMinute;
         cost = cost1 + cost2;                                 
         ttlcostserver = cost * ceil(numofserver);               //total Cost for server and using ceil function for round up value

         TotalFoodCost = guestno * CostPerDinner;               //Total cost food

         avgcost = TotalFoodCost/guestno;                       //Average cost per person
